...
class LLMClient:
    ...
    async def summarize_text(self, text: str, summary_type: str = "short") -> Optional[str]:
        ...
        if summary_type == "short":
            prompt = """以下の記事を500文字程度で要約してください。重要なポイントを簡潔にまとめてください。出力はMarkdown形式で要約した内容のみにしてください。「〜の要約」のようなタイトルは付けないでください。
...
"""
            ...
            prompt = """以下の記事を要約してください。主要な論点と詳細を含めて分かりやすくまとめてください。出力はMarkdown形式で要約した内容のみにしてください。「〜の要約」のようなタイトルは付けないでください。
