...
class LLMClient:
    ...
    async def summarize_text(self, text: str, summary_type: str = "short") -> Optional[str]:
        ...
        if summary_type == "short":
            prompt = """以下の記事を500文字程度で要約してください。重要なポイントを簡潔にまとめてください。

出力フォーマットの要件:
- 全体はMarkdown形式で出力してください。
- 重要なポイント（主要な結論やインサイト）は見出し(H2 `##`)または太字(`**...**`)で強調してください。
- 箇条書き（`-` または `*`）で要点を整理してください（各箇条は1-2文で簡潔に）。
- もし短い引用や重要な数値があれば、コードスニペット（バッククォート）かインラインコードで示してください。
- 余分な導入文や「〜の要約」のようなタイトルは付けないでください。本文のみを出力してください。
...
"""
            ...
            prompt = """以下の記事を要約してください。主要な論点と詳細を含めて分かりやすくまとめ、読み手がすぐに重要点を把握できるようにしてください。

出力フォーマットの要件:
- 全体はMarkdown形式で出力してください。
- 各セクションの冒頭に見出し(H2 `##` または H3 `###`)をつけて構造化してください。
- 重要なポイントや結論は太字(`**...**`)で強調してください。
- 詳細や裏付けは短い箇条書きで示してください。必要に応じて1-2行の短い引用（`> quote`）を使って要点を強調して構成してください。
- 余分な前置きや『〜の要約』という形式のタイトルは付けないでください。本文のみを出力してください。
