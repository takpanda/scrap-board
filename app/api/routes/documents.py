...
@router.delete("/{document_id}/personalized-feedback")
async def delete_personalized_feedback(
    request: Request,
    document_id: str,
    db: Session = Depends(get_db)
):
    """Delete "low relevance" feedback for personalized ranking."""
    ...
    # Find existing feedback for this user and document
    from app.core.database import PreferenceFeedback
    feedback = db.query(PreferenceFeedback).filter(
        PreferenceFeedback.document_id == document_id,
    ...
    if request.headers.get("HX-Request"):
        # Return the original button HTML for HTMX swap
        from html import escape
        button_html = f'''<div
    class="flex items-center"
    data-personalized-feedback-container
    data-document-id="{escape(document.id)}"
...
>
    <button
        ...
        hx-post="/api/documents/{escape(document.id)}/personalized-feedback"
    ...
    >
        ...
        response = HTMLResponse(content=button_html)
        response.headers["HX-Trigger"] = "personalized-feedback:cancelled"
        return response
